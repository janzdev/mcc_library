<aside id="sidebar" class="sidebar" id="v-pills-tab" role="tablist">
  <ul class="sidebar-nav" id="sidebar-nav">
    <li class="nav-item">
      <a class="nav-link active" href="index.php">
        <i class="bi bi-grid"></i>
        <span>Dashboard</span>
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link collapsed" href="books.php">
        <i class="bi bi-book"></i><span>Books</span>
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link collapsed" href="#">
        <i class="bi bi-people"></i><span>Borrowers</span>
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link collapsed" href="#">
        <i class="bi bi-box-arrow-up-right"></i><span>Borrowed Books</span>
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link collapsed" href="#">
        <i class="bi bi-box-arrow-in-down-left"></i><span>Return Books</span>
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link collapsed" href="#">
        <i class="bi bi-calendar2-check"></i><span>Borrowers Attendance</span>
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link collapsed" href="#">
        <i class="bi bi-info-square"></i><span>Reports</span>
      </a>
    </li>
  </ul>
</aside>