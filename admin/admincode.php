...
if (isset($_POST['borrow']))
{
   ...
   {
      echo "<script>
                  alert(' ".$allowed['qntty_books']." ".'Books Allowed per User!'." '); 
                  window.location='borrow_book.php?student_id_number=".$student_id_number."'
             </script>";
   }
   ...
   {
      echo "<script>
                  alert('Book Cannot Borrow At This Moment!'); 
                  window.location='borrow_book.php?student_id_number=".$student_id_number."'
             </script>";
   }
...
($penalty_amount['penalty_amount']));
} elseif ($date_returned < $due_date) { $penalty='No Penalty' ; } else { $penalty='No Penalty' ; }
     mysqli_query($con,"UPDATE borrow_book SET borrowed_status='returned' , date_returned='$date_returned_now' ,
     book_penalty='$penalty' WHERE borrow_book_id='$borrow_book_id' and user_id='$user_id' and book_id='$book_id' ");
   ...
   mysqli_query($con," INSERT INTO report (book_id, user_id, admin_name, detail_action, date_transaction) VALUES
     ('$book_id','$user_id','$admin_row1','Returned Book',NOW())");
     header("Location:borrow_book.php?student_id_number=$student_id_number"); } // Transaction
     if(isset($_POST['submit'])) { $student_id_number=$_POST['student_id_number'];
     $query="SELECT * FROM user WHERE student_id_no='$student_id_number' " ; $query_run=mysqli_query($con, $query);
